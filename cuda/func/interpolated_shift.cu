						  ...
						  float offsetColumn)
{
	...
	ascomplex(shry[tx][ty+1]) = 
			wx[0] * ascomplex(shr[tx][ty+1]) + 
			wx[1] * ascomplex(shr[tx+1][ty+1]) + 
			wx[2] * ascomplex(shr[tx+2][ty+1]);
	if (ty == 0) {
		ascomplex(shry[tx][0]) = 
				 wx[0] * ascomplex(shr[tx][0]) + 
	...
	if ((gx_old == 0 && topzero) || 
			(gx_old == rows-1 && bottomzero) ||
			(gy_old == 0 && leftzero) ||
			(gy_old == columns-1 && rightzero)) {
...
/** this kernel is not working and shouldn't be used. It's also not optimised at all. */
template <int BlockX, int BlockY>
...
InterpolatedShift::InterpolatedShift()
    : CudaFunction("interpolated_shift")
{
}

void InterpolatedShift::setParameters(int rows, int columns)
{
	rows_ = rows;
	columns_ = columns;
}
...
void InterpolatedShift::allocate()
{
  ...
  d_in_.allocate(rows_ * columns_);
  d_out_.allocate(rows_ * columns_);
}

void InterpolatedShift::transfer_in(const complex<float>* in)
{
  ScopedTimer t(this, "transfer in");
  gpu_memcpy_h2d(d_in_.get(), in, rows_ * columns_);
}
...
void InterpolatedShift::run(float offsetRow, float offsetColumn, bool do_linear)
{
  ...
  dim3 blocks = {
      unsigned((rows_ + 31) / 32), unsigned((columns_ + 31) / 32), 1u};
  ...
  if (std::abs(offsetRowFrac) < 1e-6f && std::abs(offsetColFrac) < 1e-6f)
  {
	  if (offsetRowInt == 0 && offsetColInt == 0) {
		  ...
		  gpu_memcpy_d2d(d_out_.get(), d_in_.get(), rows_*columns_);
	  } else {
	  ...
	  if (do_linear) {
		  linear_interpolate_kernel<32,32>
	        <<<blocks, threadsPerBlock>>>(d_in_.get(),
	        		                      d_out_.get(),
										  rows_,
										  columns_,
		...
		spline_interpolate_y<32, 32><<<blocks, threadsPerBlock>>>(d_in_.get(),
																  d_out_.get(),
																  rows_,
																  columns_,
																  offsetRow,
																  offsetColumn);
		checkLaunchErrors();
		spline_interpolate_x<32, 32><<<blocks, threadsPerBlock>>>(d_out_.get(),
																  d_in_.get(),
																  rows_,
																  columns_,
																  offsetRow,
...
void InterpolatedShift::transfer_out(complex<float>* out)
{
  ...
  gpu_memcpy_d2h(out, d_out_.get(), rows_ * columns_);
}
									 ...
									 int ido_linear)
{
  ...
  auto is = gpuManager.get_cuda_function<InterpolatedShift>("interpolated_shift", rows, columns);
  is->allocate();
  is->transfer_in(in);
