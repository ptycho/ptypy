...
RECIPE.date = None 
RECIPE.stxm_file_pattern = (
    '%(base_path)s/%(date)s/Sample_Image_%(date)s_%(scan_number_stxm)03d.hdf5')
...
I08DEFAULT = PtyScan.DEFAULT.copy()
...
class I08Scan(PtyScan):
    ...
    def __init__(self, pars=None, **kwargs):
        ...
        # Sanity check: for now we need a date to identify the STXM file
        if self.info.recipe.date is None:
            raise RuntimeError(
                'recipe.date has to be specified to find the STXM file name.')
        else:
            try: 
        ...
        self.stxm_filename = (self.info.recipe.stxm_file_pattern
                              % self.info.recipe)
    ...
    def load_common(self):
        ...
        if self.info.recipe.dark_number is not None:
            self.dark_nxs_filename = (self.info.recipe.dark_nxs_file_pattern
                                      % self.info.recipe)
            # dark = io.h5read(self.dark_nxs_filename, key)[key][0, 0, :, :]
            # this was a problem with the dark collection.
            # A 2x2 grid was collected.
            dark = io.h5read(self.dark_nxs_filename, key)[key]
            if dark.ndim == 4:
                dark.resize((dark.shape[0] * dark.shape[1],
                             dark.shape[2],
                             dark.shape[3]))
            if dark.ndim == 3:
        ...
        scan_dimensions = (io.h5read(self.nxs_filename, key)[key].shape[0],
                           io.h5read(self.nxs_filename, key)[key].shape[1])
    ...
    def load_positions(self):
        ...
        u.log(3, 'file name is: %s' % self.stxm_filename)

        x1 = io.h5read(self.stxm_filename, keyx)
        y1 = io.h5read(self.stxm_filename, keyy)

        # Grab out the positions - they are the demand positions rather than
        # read back and are not a list of co-ords.
        # Meshgrid to get almost the thing we need.
        x, y = np.meshgrid(x1[keyx], y1[keyy])
    ...
    def load(self, indices):
        ...
        # Left empty here because positions are provided by self.load_positions.
        pos = {}
        ...
        for i in range(len(indices)):
            # Find the remainder - works out the column
            ix[i] = int(np.mod(indices[i], self.common.scan_dimensions[1]))
            # Works out the row
            iy[i] = int(indices[i] // self.common.scan_dimensions[0])
            raw[i] = (io.h5read(self.nxs_filename,
                                key,
                                slice=(ix[i], iy[i]))[key].astype(np.float32)
                      - self.common.dark) / self.common.flat
            # - self.common.dark) # load in the data and convert type
