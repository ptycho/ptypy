...
NEXUS_PATHS = u.Param()
NEXUS_PATHS.frame = 'entry1/instrument/{detector_name}/data'
NEXUS_PATHS.exposure = 'entry1/instrument/{detector_name}/count_time'
...
# IO
DEFAULT.base_path = './'
DEFAULT.data_dir_pattern = '%(base_path)s' + 'raw/%(scan_number)05d.nxs'
DEFAULT.dark_dir_pattern = '%(base_path)s' + 'raw/%(dark_number)05d.nxs'
DEFAULT.flat_dir_pattern = '%(base_path)s' + 'raw/%(flat_number)05d.nxs'
...
class I13Scan(PtyScan):
    ...
    def __init__(self, pars=None, **kwargs):
        ...
        # self.info has been initialized by the parent class
        self.info.data_file = self.info.data_file_pattern % self.info
        self.info.data_file = None if self.info.dark_number is None else self.info.dark_file_pattern % self.info
        self.info.flat_file = None if self.info.flat_number is None else self.info.flat_file_pattern % self.info

    def _autofill(self):
        """
        Attempt to extract missing information about the experiment.
        """
        # Base path
    ...
    def load(self, indices):
        ...
        weights = {}
        for j in indices:
            raw[j] = io.h5read(self.info.data_file, NEXUS_PATHS.frame, slice=slice(j))
