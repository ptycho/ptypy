...
class FourierUpdateKernelTest(unittest.TestCase):
    ...
    def test_configure(self):
        ...
        #
        #
        expected_f_shape = tuple([INT_TYPE(A), INT_TYPE(B), INT_TYPE(C)])
        expected_nviews = INT_TYPE(N)
        ...
        expected_mask_sum = FLOAT_TYPE(mask.sum(-1).sum(-1))

        np.testing.assert_equal(FUK.f_shape, expected_f_shape)
        np.testing.assert_equal(FUK.nviews, expected_nviews)
        np.testing.assert_equal(FUK.nmodes, expected_nmodes)
        np.testing.assert_equal(FUK.ncoords, expected_ncoords)
        np.testing.assert_equal(FUK.naxes, expected_naxes)
        np.testing.assert_equal(FUK.num_pods, expected_num_pods)
        np.testing.assert_equal(FUK.pbound, expected_pbound)
        np.testing.assert_equal(FUK.mask_sum, expected_mask_sum)
    ...
    def test_fourier_error(self):
        ...
        mask = np.empty(shape=(N, B, C), dtype=np.int32)# the masks for the measured magnitudes either 1xAxB or NxAxB
        mask_fill = np.ones_like(mask)
        mask_fill[::2, ::2] = 0 # checkerboard for testing
        mask[:] = mask_fill
        ...
        pbound_set = 0.9
        FUK.configure(f, mask, addr, pbound=pbound_set)
        FUK.fourier_error(f, fmag, fdev, ferr, mask)  # updates fdev and ferr
    ...
    def test_error_reduce(self):
        ...
        FUK = FourierUpdateKernel()
        err_fmag = np.zeros(N, dtype=FLOAT_TYPE)
        FUK.error_reduce(ferr, err_fmag)
    ...
    def test_calc_fm(self):
        ...
        FUK.configure(f, mask, addr, pbound=pbound_set)
        FUK.fourier_error(f, fmag, fdev, ferr, mask)
        FUK.error_reduce(ferr, err_fmag)
        FUK.calc_fm(fm, mask, fmag, fdev, err_fmag)
