...
class _StochasticEngine_ref(PositionCorrectionEngine):
        ...
        for it in range(num):
            ...
            for name in vieworder:
                ...
                for name, pod in view.pods.items():
                    ...
                    exit_wave[name] = np.exp(1j * pod.object) * pod.probe
    ...
    def fourier_update(self, view):
        ...
        err_fmag, err_exit = projection_update_generalized_ref(view, self._a, self._b, self._c)
    ...
    def _generic_object_update(self, view, exit_wave, a=0., b=1.):
        ...
        for name, pod in view.pods.items():
            ...
            pod.object += (a + b) * np.conj(1j * pod.probe * np.exp(1j * pod.object))  * (pod.exit - exit_wave[name]) / probe_norm
    ...
    def _generic_probe_update(self, view, exit_wave, a=0., b=1.):
        ...
        # Calculate the object norm based on the local object
            ...
            for name, pod in view.pods.items():
                ...
                object_power += u.abs2(np.exp(1j * pod.object))
            object_norm = (1 - a) * np.max(object_power) + a * object_power
        for name, pod in view.pods.items():
            #pod.probe += (a + b) * np.conj(pod.object) * (pod.exit - exit_wave[name]) / object_norm
            pod.probe += (a + b) * np.exp(-1j * np.conj(pod.object)) * (pod.exit - exit_wave[name]) / object_norm
...
@register()
class EPIE_ref(_StochasticEngine_ref, EPIEMixin_ref):
    """
    The ePIE algorithm.
    ...
    """
    def __init__(self, ptycho_parent, pars=None):
        _StochasticEngine_ref.__init__(self, ptycho_parent, pars)
        EPIEMixin_ref.__init__(self, self.p.alpha, self.p.beta)
        ptycho_parent.citations.add_article(**self.article)


@register()
class SDR_ref(_StochasticEngine_ref, SDRMixin_ref):
    """
    ...
    def __init__(self, ptycho_parent, pars=None):
        _StochasticEngine_ref.__init__(self, ptycho_parent, pars)
        SDRMixin_ref.__init__(self, self.p.sigma, self.p.tau, self.p.beta_probe, self.p.beta_object)
        ptycho_parent.citations.add_article(**self.article)
