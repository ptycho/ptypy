                                      ...
                                      const int* addr)
{
  ...
  __shared__ int addresses[BDIM_X*BDIM_Y*7];
  ...
  for (int p = 0; p < num_pods; p += BDIM_X*BDIM_Y)
  {
    ...
    if (txy < mi) {
      ...
      addresses[txy*7+0] = pr_dlayer(p+txy);
      addresses[txy*7+1] = ex_dlayer(p+txy);
      addresses[txy*7+2] = obj_dlayer(p+txy);
      assert(obj_dlayer(p+txy) < NUM_MODES);
      assert(addresses[txy*5+2] < NUM_MODES);
      addresses[txy*7+3] = pr_roi_row(p+txy);
      addresses[txy*7+4] = pr_roi_column(p+txy);
      addresses[txy*7+5] = obj_roi_row(p+txy);
      addresses[txy*7+6] = obj_roi_column(p+txy);
    }
    ...
    #pragma unroll 4
    for (int i = 0; i < mi; ++i){
      int* ad = addresses + i*7;
      int v1 = y - ad[3];
      int v2 = z - ad[4];
      if (v1 >= 0 && v1 < ob_sh_row && v2 >= 0 && v2 < ob_sh_col) {
        
        auto obidx = ad[2] * ob_sh_row * ob_sh_col + (v1+ad[5]) * ob_sh_col + (v2+ad[6]);
