/*
This was a test to join the fourier update kernels, but the performance
                                         ...
                                         )
{
    ...
    auto shmt = shm + threadIdx.x * blockDim.y * blockDim.z + threadIdx.y * blockDim.z;

    // modes values
    if (ix < B && iy < A) {
        float abs_exit_wave = abs(f[tz*A*B]);
        shmt[tz] = abs_exit_wave * abs_exit_wave; // if we do this manually (real*real +imag*imag) we get bad rounding errors
    } else {
        shmt[tz] = 0.0f;
    }
    __syncthreads();
    ...
    if (ix < B && iy < A && renorm < 1.0f) {
        ...
        auto fmagv = *fmag;
        auto fdevv = *fdev;
        float fm = (1.0f - m) + m * ((fmagv + fdevv * renorm) / (fmagv + fdevv + 1e-10f)) ;
        *f *= fm;
    }
}
