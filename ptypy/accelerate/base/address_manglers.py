...
class BaseMangler(object):
    ...
    def __init__(self, max_step_per_shift,  start, stop, nshifts, decay=True, max_bound=None,  randomseed=None):
        ...
        self.max_step = lambda it: np.ceil(max_step_per_shift * (stop - it) / (stop - start)) if decay else max_step_per_shift # maximum step per iteration, decreases with progression
