...
extern "C"
{
  __global__ void pr_update_ML_wavefield(const complex<IN_TYPE>* __restrict__ exit_wave,
                                                  int A,
                                                  int B,
                                                  int C,
                                                  ...
                                                  IN_TYPE fac_)
  {
    ...
    for (int b = ty; b < B; b += blockDim.y)
    {
      for (int c = tx; c < C; c += blockDim.x)
      {
        ...
        complex<MATH_TYPE> add_val_m2 = conj(obj_val) * obj_val;
        complex<OUT_TYPE> add_val2 = add_val_m2;
