...
__all__ = ['MPLClient', 'MPLplotter', 'PlotClient', 'spawn_MPLClient',
           'TEMPLATES', 'DEFAULT']
...
if __name__ == "__main__":
    ...
    from ptypy.utils.plot_utils import plt
    # pyplot import with all the specialized settings
...
Storage_DEFAULT = TreeDict(
    'Storage_DEFAULT',
    # maybe we would want this container specific
    clims=[None, [-np.pi, np.pi]],
    cmaps=['gray', 'hsv'],
    # fraction of array to crop for display
    crop=[0.3, 0.3],
    # remove_phase_ramp = True
    rm_pr=True,
    # if None the shape is determining
    shape=None,
    # quantities to display
    auto_display=['a', 'p'],
    # (int or list or None)
    layers=None,
    # plot a local error map (ignored in probe)
    local_error=False,
    use_colorbar=True,
    # Fraction (radius) of data to use for clims (if None) or phase_ramp removal
    mask=0.3,
)

DEFAULT = TreeDict('DEFAULT')
DEFAULT.figsize = (16, 10)
DEFAULT.attach('ob', Storage_DEFAULT)
DEFAULT.attach('pr', Storage_DEFAULT)
...
class PlotClient(object):
    ...
    be a waste of bandwidth if all that is required is a client that plots
    "on demand"...
    ...
    def __init__(self, client_pars=None, in_thread=False):
        """
        Create a client and attempt to connect to a running reconstruction
        server.
        """
        ...
        # If client_pars is None, connect with the defaults
        # defined in ptypy.io.interaction
        self.client = Client(client_pars)

        # Data requests are stored in this dictionary:
        # self.cmd_dct['cmd'] = [ticket, buffer, key]
        # When the data associated with a ticket arrives
        # it is placed in buffer[key].
        self.cmd_dct = {}

        # Initialize data containers.
        # Here we use our own "Param" class, which adds attribute access
        # On top of dictionary.
        # Probe
        self.pr = Param()
        # Object
        self.ob = Param()
        # Runtime information (contains reconstruction metrics)
        self.runtime = Param()
    ...
    def _initialize(self):
        """
        Wait for reconstruction to start, then grab synchronously basic
        ...
        log(self.log_level,
            '1 object to plot.' if len(ob_IDs) == 1
            else '%d objects to plot.' % len(ob_IDs))
        ...
        for ID in ob_IDs:
            ...
            self.ob[ID] = S
            self.cmd_dct[
                "Ptycho.obj.S['%s'].data" % str(ID)] = [None, S, 'data']
            self.cmd_dct[
                "Ptycho.obj.S['%s'].psize" % str(ID)] = [None, S, 'psize']
            self.cmd_dct[
                "Ptycho.obj.S['%s'].center" % str(ID)] = [None, S, 'center']

        # Get the list of probe IDs
        pr_IDs = self.client.get_now("Ptycho.probe.S.keys()")
        log(self.log_level,
            '1 probe to plot.' if len(pr_IDs) == 1
            else '%d probes to plot.' % len(pr_IDs))
        ...
        for ID in pr_IDs:
            ...
            self.pr[ID] = S
            self.cmd_dct[
                "Ptycho.probe.S['%s'].data" % str(ID)] = [None, S, 'data']
            self.cmd_dct[
                "Ptycho.probe.S['%s'].psize" % str(ID)] = [None, S, 'psize']
            self.cmd_dct[
                "Ptycho.probe.S['%s'].center" % str(ID)] = [None, S, 'center']

        # Data request for the error.
        self.cmd_dct[
            "Ptycho.runtime['iter_info'][-1]"] = [None,
                                                  self.runtime,
                                                  'last_info']

        # Get the dump file path
        self.cmd_dct[
            "Ptycho.paths.plot_file(Ptycho.runtime)"] = [None,
                                                         self.runtime,
                                                         'plot_file']
        
        # Get info if it's all over
        self.cmd_dct[
            "Ptycho.runtime.get('allstop') is not None"] = [None,
                                                            self.__dict__,
                                                            '_stopping']
    ...
    def _store_data(self):
        ...
        with self._lock:
            ...
            # An extra step for the error.
            # This should be handled differently at some point.
            # self.error = np.array([info['error'].sum(0)
            #                        for info in self.runtime.iter_info])
            self._new_data = True
        self.client.flush()
...
class MPLplotter(object):
    ...
    def __init__(self, pars=None, probes=None, objects=None, runtime=None,
        ...
        self.ob = objects
        if runtime is None:
            self.runtime = TreeDict('self.runtime')
    ...
    def _set_autolayout(self, pars):
        self.p = TreeDict('self.p')
        self.p.update(self.DEFAULT)
    ...
    def update_plot_layout(self, plot_template=None):
        ...
        for key in sorted(self.ob.keys()):
            ...
            num_shape = [len(layers) * len(plot.auto_display)
                         + int(plot.local_error), sh]
            num_shape_list.append(num_shape)
            self.ob_plot[key] = plot
            
        # Per default we will use the a frame similar to
        # the last object frame for plotting
        if np.array(self.p.plot_error).any():
            self.error_axes_index = len(num_shape_list) - 1
            self.error_frame = num_shape_list[-1][0]
            # Add a frame
            num_shape_list[-1][0] += 1
        ...
        axes_list, plot_fig, gs = self.create_plot_from_tile_list(
            1, num_shape_list, self.p.figsize)
    ...
    @staticmethod
    def create_plot_from_tile_list(fignum=1, num_shape_list=[(4, (2, 2))],
                                   figsize=(8, 8)):
            ...
            while num > 0:
                ...
                if num <= N_v and np.abs(N_h - num) >= np.abs(N_v - num):
                    ...
                    Horizontal = True
                      ...
                      or size[1] / float(size[0] + 0.00001) > figratio):
                    Horizontal = True
                else:
                    Horizontal = False
        ...
        for ii, (N, sh) in enumerate(num_shape_list):
            if not (np.allclose(np.asarray(sh), np.asarray(last_sh))
                    or ii == (len(num_shape_list)-1)):
                nsh = np.array(last_sh) + frame
                coords, size = fill_with_tiles(size,
                                               nsh,
                                               M + N,
                                               fig_aspect_ratio)
                M = 0
        ...
        space = 0.1 * size[0]
        gs.update(wspace=0.1 * size[0],
                  hspace=0.12 * size[0],
                  left=0.07,
                  right=0.95,
                  bottom=0.05,
                  top=0.93)  # this is still a stupid hardwired parameter

        axes_list = []
        for (N, sh), coords in zip(num_shape_list, coords_list):
            axes_list.append(
                [fig.add_subplot(
                    gs[co[0] + frame // 2:co[0] + frame // 2 + sh[0],
                        co[1] + frame // 2:co[1] + frame // 2 + sh[1]])
                 for co in coords])
    ...
    def plot_error(self):
        if np.array(self.p.plot_error).any():
            try:
                ...
                axis.plot(fmag,
                          label='err_fmag %2.2f%% of %.2e'
                                % (fmag[-1] * 100, np.max(err_fmag)))
                axis.hold(True)
                phot = err_phot/np.max(err_phot)
                axis.plot(phot,
                          label='err_phot %2.2f%% of %.2e'
                                % (phot[-1] * 100, np.max(err_phot)))
                ex = err_exit/np.max(err_exit)
                axis.plot(ex,
                          label='err_exit %2.2f%% of %.2e'
                                % (ex[-1] * 100, np.max(err_exit)))
                axis.legend(loc=1, fontsize=10)
                # ('err_fmag %.2e' % np.max(err_fmag),
                #  'err_phot %.2e' % np.max(err_phot),
                #  'err_exit %.2e' % np.max(err_exit)),
    ...
    def plot_storage(self, storage, plot_pars, title="", typ='obj'):
        ...
        if np.isscalar(pp.mask):
            x, y = np.indices(sh) - np.reshape(
                np.array(sh) // 2, (len(sh),) + len(sh) * (1,))
        ...
        for layer in pp.layers:
            for ind, channel in enumerate(pp.auto_display):
                ...
                try:
                    ...
                    cmap = pp.cmaps[ind % len(pp.cmaps)]
                    # if ind[1]=='p' else pp.cmaps[0]
                    ptya = PtyAxis(axes[ii],
                                   data=data[layer],
                                   channel=channel,
                                   cmap=cmap)
                    ...
                    if pp.clims is not None and pp.clims[ind] is not None:
                        ptya.set_clims(pp.clims[ind][0],
                                       pp.clims[ind][1],
                                       False)

                    if pp.use_colorbar:
                        ptya.add_colorbar()
                    ptya._resize_cid = self.plot_fig.canvas.mpl_connect(
                        'resize_event', ptya._after_resize_event)
    ...
    def save(self, pattern, count=0):
        try:
            ...
            log(self.log_level,
                'Could not auto generate image dump file from runtime.')
...
class MPLClient(MPLplotter):
    ...
    def __init__(self, client_pars=None, autoplot_pars=None, layout_pars=None,
                 in_thread=False, is_slave=False):
        
        from ptypy.core.ptycho import DEFAULT_autoplot
        self.config = TreeDict('self.config')
        self.config.update(DEFAULT_autoplot)
...
if __name__ == '__main__':
    ...
    pr = Param({'Moon': Param({'data': dmoon,
                               'shape': dmoon.shape,
                               'psize': 1})})
    ob = Param({'Flower':Param({'data': dflower,
                                'shape': dflower.shape,
                                'psize': 1})})
    runtime = Param()
    runtime.iter_info = []
