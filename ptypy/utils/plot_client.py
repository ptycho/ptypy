...
class _MPLClient(object):
    """
    **DEPRECATED** Plotting Client for Ptypy, using matplotlib.
    """
    DEFAULT = DEFAULT
    ...
    def update_plot_layout(self, plot_template=None):
        ...
        for key in sorted(pr.keys()):
            ...
            cp.layers = layers
            cp.axes_index = len(num_shape_list)
            num_shape = [len(layers)*len(cp.auto_display), sh]
            num_shape_list.append(num_shape)
            self.ob_plot[key]=cp
        ...
        for ii, ind in enumerate([(l, a) for l in pp.layers for a in pp.auto_display]):
            ...
            if ind[1] == 'c':
                ...
                if not axes[ii].images:
                    ...
                    plt.setp(axes[ii].get_xticklabels(), fontsize=8)
                    plt.setp(axes[ii].get_yticklabels(), fontsize=8)
                else:
            ...
            if ind[1] == 'p':
                ...
                cmap = plt.get_cmap(pp.cmaps[1])
                clims = pp.clims[1]
            elif ind[1] == 'a':
                d = np.abs(ndat)
                ttl = '%s#%d (M)\n%s' % (title, ind[0], info)
                cmap = plt.get_cmap(pp.cmaps[0])
                clims = pp.clims[0]
