...
class AstraViewBased:
    ...
    def __init__(self, vol, n_views, view_shape, block_size, angles, shifts, view_to_proj_vectors):
        ...
        """
        vol                     3d array
        n_views                 number of views overall, considering all blocks
        view_shape              shape of view
        block_size              size of chunk (a sub-part or equal to n_views) -- how many views you want to keep in memory
        angles                  1d array of angles (same length as n_views)
        shifts                  2d array of x-y shifts (same length as n_views, while other direction has len 2, i.e. x and y)
        view_to_proj_vectors    2d array of vectors computed as differences between the center of views and the center of projections (same length as n_views)
        """
    ...
    def proj_array(self):
        """
        Has length "block_size" -- could be full list of views or smaller block.
        """
    ...
    def ind_of_views(self):
        """
        Indices of views in the block. 
        Has length of proj_array and so of "block_size".
        These need to be ordered and correspond to proj_array.
        """
    ...
    def vol(self, vol):
        """
        Sets the volume and updates data at id
        """   
        self._vol = vol
    ...
    def _update_data_at_id_astra(self, id, data):
        """
        Helper function that updates data at a certain id
        """
        astra.data3d.store(id, data)

    def _update_geom_at_id_astra(self, id, new_geom):
        """
        Helper function that updates geometry at a certain id
        """
    ...
    def _create_proj_array_ids(self):
        """
        Creates proj array ids.
        _create_geom needs to be called before this. 
    ...
    def _create_vol_ids(self):
        """
        Creates vol ids.
        _create_vol_geom needs to be called before this. 
        This function is called once only, when the class is initialised.        
        """
        self._vol_id_real = astra.data3d.create("-vol", self._vol_geom, self._vol.real)
        self._vol_id_imag = astra.data3d.create("-vol", self._vol_geom, self._vol.imag)
    ...
    def _create_vector_for_proj_geom(self):
        """
        Creates vector needed to construct proj array geometry
        """
        self._vec = np.zeros((self._n_views,12)) # size 1444x12, even with mpi
    ...
    def forward(self, iter=1, out=None):
        """
        Computes forward projection, based on sel._vol (this must have been defined).
        Places output in "out" if provided
        """
    ...
    def backward(self, iter=1, out=None):
        """
        Computes backward projection, based on _proj_array (this must have been defined).
        Places output in "out" if provided.
        """
