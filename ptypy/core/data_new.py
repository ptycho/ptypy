...
class PtyScan(object):
    """
    PtyScan: A single ptychography scan
    ...
    *BASECLASS*
    ...
    def __init__(self, pars=None, **kwargs):
        ...
        # Store all input parameters for documentation purposes.
        self.info = p
    ...
    def initialize(self):
        ...
        * Calls :py:meth:`post_initialize`
        ...
        if self.save is not None and parallel.master:
            ...
            io.h5append(self.dfile, info=dict(self.info))
        # Wait for master
        parallel.barrier()
    ...
    def load_weight(self):
        ...
        *Called in* :py:meth:`initialize`
        ...
        if self.shape is None:
            ...
            return np.ones(u.expect2(self.shape), dtype='bool')
    ...
    def get_data_chunk(self, chunksize, start=None):
        ...
        if msg in [EOS, WAIT]:
            ...
            self.center = cen / float(self.rebin)
            self.shape = u.expect2(sh) / self.rebin

            if self.info.psize is not None:
                self.psize = u.expect2(self.info.psize) * self.rebin
    ...
    def _make_data_package(self, chunk):
        ...
        # The "common" part
        common = u.Param({k: self.info[k] for k in self.METAKEYS})
        out = {'common': common}
...
class PtyScanDynamic(PtyScan):
    """
    PtyScanDynamic: A single ptychography scan fed on the fly
    ...
    def __init__(self, pars=None, **kwargs):
        ...
        Call :py:data:`initialize` to begin loading and data file creation.
        """
        super(PtyScanDynamic, self).__init__(pars, **kwargs)
