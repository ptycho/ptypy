...
DEFAULT_aperture = TreeDict(
    'DEFAULT_aperture',
    form='circ',
    # (str) One of None, 'rect' or 'circ'
    #  One of:
    # - None: no aperture, this may be useful for nearfield
    # - 'rect': rectangular aperture
    # - 'circ': circular aperture
    diffuser=None,
    # (float) Static Noise in the transparent part of the aperture
    ...
    size=None,
    # (float) Aperture width or diameter
    # May also be a tuple (vertical,horizontal) for size in case of an
    # asymmetric aperture
    edge=2,
    # (int) Edge width of aperture in pixel to suppress aliasing
    central_stop=None,
    # (float) size of central stop as a fraction of aperture.size
    # If not None: places a central beam stop in aperture.
    #  The value given here is the fraction of the stop compared to size
    offset=0.,
    # (float) offset between center of aperture and optical axes
    # May also be a tuple (vertical,horizontal) for size in case of an
    # asymmetric offset
    rotate=0.,
...
""" Default illumination.aperture parameters.
    See :py:data:`.scan.illumination.aperture`
    and a short listing below """

DEFAULT_propagation = TreeDict(
    'DEFAULT_propagation',
...
DEFAULT = TreeDict(
    ...
    # `None`, path to a *.ptyr file or any python evaluable statement yielding
    # a 3d numpy array. If `None` illumination is modeled.
    photons=None,
    # (int,float,None) Number of photons in the incident illumination
)

DEFAULT.attach('recon', RECON)
DEFAULT.attach('stxm', STXM)
# Diversity parameters, can be None = no diversity
DEFAULT.attach('diversity', DEFAULT_diversity)
# Aperture parameters, can be None = no aperture
DEFAULT.attach('aperture', DEFAULT_aperture)
# Propagation parameters, can be None = no propagation
DEFAULT.attach('propagation', DEFAULT_propagation)

""" Default illumination parameters.
    See :py:data:`.scan.illumination`
    and a short listing below """
...
def init_storage(storage, pars, energy=None, **kwargs):
    ...
    pars : Param
         ...
         * string giving the filename of a previous reconstruction to extract
           storage from.
    ...
    # First we check for scripting shortcuts. This is only convenience
        ...
        if pars.endswith('.ptyr'):
            recon = TreeDict('recon', rfile=pars, layer=None, ID=s.ID)
            p.attach('recon', recon)
            p.model = 'recon'
            try:
    ...
    if p.model is None:
        ...
        # SC: probably KeyError will have to go here,
        # but needs testing with data
        except:
...
def _process(model, aperture_pars=None, prop_pars=None, photons=1e7, energy=6.,
             resolution=7e-8, prefix="", **kwargs):
    """
