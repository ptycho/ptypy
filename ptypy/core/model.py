...
class BasicSharingModel(object):
    ...
    def __init__(self, sharing_dct, scan_per_probe, scan_per_object, npts=None):
        ...
        if scan_per_probe == 0:
            ...
            u.logger.info('Sharing a single probe for ALL scans.')
        elif scan_per_probe >= 1:
            self.shared_probe = True
            self.single_probe = False
            self.scan_per_probe = int(scan_per_probe)
            u.logger.info('Model: sharing probe between scans '
                          '(one new probe every %d scan)'
                          % self.scan_per_probe)
        else:
            ...
            self.npts = npts
            u.logger.info('Model: splitting scans '
                          '(every %d diffraction patter)'
                          % self.diff_per_probe)
        ...
        if scan_per_object == 0:
            ...
            u.logger.info('Sharing a single object for ALL scans.')
        elif scan_per_object >= 1:
            self.single_object = False
            self.shared_object = True
            self.scan_per_object = int(scan_per_object)
            u.logger.info('Model: sharing object between scans '
                          '(one new object every %d scan)'
                          % self.scan_per_object)
        else:
            raise RuntimeError(
    ...
    def __call__(self, scan_label, diff_index):
        ...
        u.logger.debug("Model assigned frame %d of scan %s to probe %s "
                       "& object %s"
