...
DEFAULT_process = TreeDict(
    'DEFAULT_process',
    offset=0,
    # (float,tuple) Offset between center of object array and
    # scan pattern (in pixel)
    zoom=None,
    # (float,tuple) Zoom value for object simulation.
    formula=None,
    # (str) Chemical formula
    density=None,
    # (None,float) Density in [g/ccm]
    thickness=None,
    # (float) Maximum thickness of sample in meter
    ref_index=0.5 + 0.j,
    # (complex float) Assigned refractive index (maximum) relative to air
    smoothing=2,
    # (float,tuple) Gaussian filter smoothing with this FWHM (pixel)
)
...
DEFAULT = TreeDict(
    ...
    model=None,
    # (None,'sim','stxm','recon')Type of object model
    fill=1.0,
)
...
DEFAULT.attach('process', DEFAULT_process)
...
def init_storage(storage, sample_pars=None, energy=None):
    ...
    # First we check for scripting shortcuts. This is only convenience
        ...
        if sam.endswith('.ptyr'):
            recon = TreeDict('recon', rfile=sam, layer=None, ID=s.ID)
...
def simulate(A, pars, energy, fill=1.0, prefix="", **kwargs):
    ...
    return np.array(
        obj[..., off[0]:off[0] + shape[0], off[1]:off[1] + shape[1]])
...
DEFAULT_old = u.Param(
    ...
    thickness=1e-6,
    # max thickness of sample if None, the absolute values of loaded
    # src array will be used
    smoothing_mfs=None,
    ...
    noise_mfs=None,
    fill=1.0,
    # if object is smaller than the objectframe, fill with fill:
    obj=None,
    # override
