...
def from_pars(xypars=None):
    ...
    if xypars == 'empty' or None:
        logger.debug('No parameters for scan pattern were provided.\n'
                     'Will use positions provided by meta information.')
        return None
    elif hasattr(xypars, 'items') or hasattr(xypars, 'iteritems'):
        # This is a dict
        p.update(xypars)
        ...
        ValueError(
            'Input type `%s` for scan pattern creation is not understood'
            % str(type(xypars)))
