...
p.model.geometry.N = 1024                         # (22) Number of detector pixels
p.model.geometry.prop_type = "nearfield"           # (23) propagation type

p.model.if_conflict_use_meta = True

p.model.xy = u.Param()
p.model.xy.scan_type = 'custom'                # (25) None,'round', 'raster', 'round_roi','custom'
p.model.xy.dr = 2e-6                             # (26) round,round_roi :width of shell
...
p.model.xy.positions = u.parallel.MPIrand_uniform(0.0,10e-6,(20,2))                       # (35) override
#p.model.xy.positions = np.random.normal(0.0,3e-6,(20,2))
...
p.model.illumination.aperture_size = 90e-6        # (42) aperture diameter (meter)
...
p.model.illumination.photons = 1e11                # (49) number of photons in illumination
p.model.illumination.probe = None                 # (50) override if not None

p.model.sample = u.Param()
p.model.sample.source = u.xradia_star((1500,1500),minfeature=3,contrast=0.8)# (52) 'diffraction', None, path to a file or nd-array 
...
p.model.sample.formula = "Au"                     # (55) chemical formula (string)
p.model.sample.density = 19.3                     # (56) density in [g/ccm]
p.model.sample.thickness = 700e-9                 # (57) max thickness of sample
