...
This version upgrades working_version.py to include modes for both the probe and the object.
...
do_recon = True
do_simulation = True
initialise_object_storages = True
...
def basic_fourier_update(diff_view, mode_lookup, alpha=1.):
    # Prepare dict for storing propagated waves


    # Get measured data
    I = diff_view.data
    af2 = np.zeros_like(di_view.pod.diff)
    f = {}
    # for mode_idx, mode in mode_lookup[di_view.ID].iteritems():
    #     upstream_pod = mode[0]
    #     downstream_pod = mode[-1]
    ...
    #     f[mode_idx] = downstream_pod.fw(downstream_pod.exit)
    #     Idata += u.abs2(f[mode_idx])
    # Get the mask
    ...
    for mode_idx, mode in mode_lookup[di_view.ID].iteritems():
        ...
        upstream_pod.exit = upstream_pod.object * upstream_pod.probe
        downstream_pod.probe = upstream_pod.fw(upstream_pod.exit)
        f[mode_idx] = downstream_pod.fw((1 + alpha) * downstream_pod.probe * downstream_pod.object - alpha * downstream_pod.exit)
        af2 += u.abs2(f[mode_idx])
    ...
    for mode_idx, mode in mode_lookup[di_view.ID].iteritems():
        downstream_pod = mode[1]
        df = downstream_pod.bw(fm * f[mode_idx]) - downstream_pod.probe * downstream_pod.object
...
for i in range(len(di_views)):
    ...
    pos_obj = positions[i]
    mode_lookup[dv.ID] = {}
    for pi in range(num_probe_modes):
        for oi in range(num_object_modes):
            mode_idx = pi + oi + pi * (num_object_modes-1)
            mode_lookup[dv.ID][mode_idx] = []
    ...
    for slice_idx, geo in geometries.iteritems():
        ...
        for geometry_index, geometry in enumerate(geo):
            ...
            for pm in range(num_probe_modes):
                for om in range(num_object_modes):
                    # print("di_index: %s, slice idx:%s, geometry index:%s, %s, probe_idx:%s, obj_index:%s" % (i, slice_idx, geometry_index, geometry.propagator, pm, om))
                    exit_index = index * 10000 + pm * 100 + om
                    mode_idx = pm + om + pm * (num_object_modes -1)
                    # contents
                    if slice_idx == 0:
                    ...
                    # new_pods.append(pod)
                    mode_lookup[dv.ID][mode_idx].append(pod)
...
if do_simulation:
    ...
    for dname, di_view in enumerate(di_views):
        ...
        Idata = np.zeros_like(di_view.pod.diff)
        f={}
        for mode_idx, mode in mode_lookup[di_view.ID].iteritems():
            upstream_pod = mode[0]
            downstream_pod = mode[-1]
...
if do_recon:
    ...
    for it in range(num_its):
        ...
        for dname, di_view in P.diff.views.iteritems():
            basic_fourier_update(di_view, mode_lookup, alpha=alpha)
