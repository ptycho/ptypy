...
FILE = 'Desktop/new_server/frontend2.html'
...
def test_app(environ, start_response):
    if environ['REQUEST_METHOD'] == 'POST':                 #If we recieve a "POST" request from the webpage 
        ...
        #Define the type of content being sent to the browswer and return it                            
        status = '200 OK'
        headers = [('Content-type', 'image/svg+xml'), ('Content-Length', str(len(response_body)))]
        start_response(status, headers)
        return [response_body]    
    
    #If we recieve any other type of request such as a "GET" request, re-open the html file.
    else:
